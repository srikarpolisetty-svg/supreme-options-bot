...
def ingest_option_snapshot_5w(symbol: str, shard_id: int):
    ...
    run_id = snapshot_id  # local run id (no params)
...
def master_ingest_5w(results, db_path: str = "options_data.db"):
    assert results and isinstance(results, list), "results must be a non-empty list of manifests"
    ...
    try:
        ...
        for r in results:
            # basic safety
            run_id = r.get("run_id")
            raw_dir = r.get("raw_dir")
            enriched_dir = r.get("enriched_dir")
            signals_dir = r.get("signals_dir")

            assert run_id is not None, "manifest missing run_id"
            assert raw_dir and enriched_dir and signals_dir, f"manifest missing dirs for run_id={run_id}"
